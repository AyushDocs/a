...
const ServerFetch=async(url:string,body:User|User[])=>{
   const options:any = {method: "POST",credentials:'include',headers:{"Content-Type": "application/json"},body:JSON.stringify(body)}
     return  fetch(`http://localhost:8080/api${url}`,options)
}

export const adminLogin=async(email:string, password:string)=>{
   const body=[{password, email}]
   const res=await ServerFetch('/users/admin/login',body)
...
export const login=async(email:string, password:string)=>{
   const body={password, email}
   const res=await ServerFetch('/users/login',body)
   const data=await res.json();
...
export const signup=async(email:string, password:string)=>{
   const body={password, email}
   const res=await ServerFetch('/users/signup',body)
   const data=await res.json();
